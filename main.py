...
panel_keypair = panel_keypair = {"패널아이디" : "패널비번"}
                    ...
                    if not (os.path.isfile(db(request.form["url"]))):
                        ...
                        if (captcha_result["success"] == True):
                            ...
                            with con:
                                ...
                                con.commit()
                                cur.execute("""INSERT INTO info VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (request.form["name"], "", "", "", "[]", "[]", "", make_expiretime(license_result[1]), "", "공지가 없습니다.", 0, "", "", 0 if license_result[5] == 0 else 1, "", "", "", "https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", "사용함", "", "", "", ""))
                                con.commit()
                    ...
                    if ("name" in request.get_json() and "amount" in request.get_json() and request.get_json()["amount"].isdigit()):
                        ...
                        if (bank_addr != ""):
                            ...
                            if (server_info[19] != None):
                                try:
                                    server_info = get_info(name)
                                    webhook = DiscordWebhook(username="Jihun Web", avatar_url="https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", url=server_info[19])
